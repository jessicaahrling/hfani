...
// ============================================================
const Y5S9={fz:0};   // fz: scentid då kameran zoomar in helt på ögat (0 = ingen slutzoom)
...
function Y5_openai(t){const S=Y5S9,[cx,cy]=S.C,[fx,fy,fw,fh]=S.F;
  const zk=E.io(seg(t,16.6,18.0)),fz=S.fz?E.io(seg(t,S.fz,S.fz+2.8)):0;
  if(zk>0)setCam(lerp(960,1620,zk),lerp(540,200,zk),lerp(1,2.6,zk)*Math.exp(Math.log(9.5/2.6)*fz),960,lerp(lerp(540,500,zk),540,fz));
...
// ============================================================
const Y5S11={nd:1.4};   // nd: hur länge efter att kurvan fällts ut som nätverket börjar växa
...
function Y5_slut(t){const S=Y5S11,[px0,py0]=S.P0,tn=t-S.nd;   // tn: nätverkets klocka, S.nd s efter kurvan
  ...
  // the curve
  const dim=lerp(1,.3,E.io(seg(tn,10.5,13.5)));
  ...
  if(tn>8.7){for(const k of [M,B]){k.save();k.globalCompositeOperation='lighter';k.strokeStyle=OR;k.lineWidth=1.5;k.lineCap='round';k.setLineDash([]);k.globalAlpha=k===M?.3:.14;
      for(let g=0;g<6;g++){k.beginPath();for(let q=g;q<S.edges.length;q+=6){const [a,b]=S.edges[q];if(a.tb>tn)continue;const e=E.o(seg(tn,a.tb,a.tb+.45));k.moveTo(b.x,b.y);k.lineTo(lerp(b.x,a.x,e),lerp(b.y,a.y,e));}k.stroke();}k.restore();}
    for(const [a,b] of S.thick){if(a.tb>tn+.6)continue;const e=E.io(seg(tn,a.tb-.6,a.tb));line(b.x,b.y,a.x,a.y,2.8,{c:OR,a:.6,p:e});
      if(e>=1){const f=((t*.45+a.ph)%1);dot(lerp(b.x,a.x,f),lerp(b.y,a.y,f),3.6,{c:HOT,a:Math.sin(f*Math.PI)});}}
    const settled=[];
    for(const n of S.nodes){if(n.tb>tn)continue;const f=seg(tn,n.tb,n.tb+.4),w=1+.2*Math.sin(t*2.2+n.ph);
      if(n.hub){D('#000',1,0,k=>{k.fillStyle='#000';k.beginPath();k.arc(n.x,n.y,n.rr+9,0,TAU);k.fill();});dot(n.x,n.y,n.rr*E.ob(f),{c:OR});ring(n.x,n.y,(n.rr+9)*w,2,{c:OR,a:.7});ringPulse(n.x,n.y,tn,n.tb,.9,n.rr,n.rr+60,2.4,{c:OR});}
  ...
  if(dp>0){dot(px0,py0,13*dp,{c:OR});ring(px0,py0,24+3*Math.sin(t*3),2.4,{c:OR,a:.7*dp});
    const la=E.io(seg(t,5.0,5.7))*lerp(1,0,seg(tn,11.5,13));text('i dag',px0+2,py0-46,58,{a:la,al:'center',e:.12});
    const ru=E.io(seg(tn,8.9,12.0));if(ru>0&&ru<1){const h=S.cv(lerp(S.u0,1,ru));dot(h[0],h[1],lerp(13,34,ru),{c:OR});dot(h[0],h[1],lerp(6,16,ru),{c:HOT});}}
}
